...
static void NVIC_Configuration(void) //USB_Interrupt 里面虽然有，但这里还是先保留
{
	//SDIO NVIC level : Highest
	...
	//EXTI NVIC PA0 button interrupt
	...
	//EXTI NVIC PC13 button interrupt
...
void SD_DMA_RxConfig(uint32_t *BufferDST, uint32_t BufferSize)
{
	...
	DMA_ClearFlag(DMA2_FLAG_GL4 | DMA2_FLAG_TC4 | DMA2_FLAG_HT4 | DMA2_FLAG_TE4);
	DMA_Cmd(DMA2_Channel4,DISABLE);
	
	DMA_InitStructure.DMA_BufferSize = BufferSize/4;
	DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralSRC;
	DMA_InitStructure.DMA_M2M = DMA_M2M_Disable;
	DMA_InitStructure.DMA_MemoryBaseAddr = (uint32_t)BufferDST; 					//ALARM 
	DMA_InitStructure.DMA_MemoryDataSize = DMA_MemoryDataSize_Word;
	DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;
	DMA_InitStructure.DMA_Mode = DMA_Mode_Normal;
	DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)SDIO_FIFO_ADDRESS;//ALARM
